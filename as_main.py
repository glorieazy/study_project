...
def main():
    ...
    # Divide frames 
    #finding the number of frames for primary video
    # folder path
    dir_path = './frames'
    count = 0
    # Iterate directory
    ...
    #secondary video
    for index in range(end_index,frames_folder.count+1):
                    ...
                    if right_wrist_landmark_1 and right_wrist_landmark_2:
                        ...
                        while dx > 0.05 | dx < -0.05 | dy > 0.05 | dy < -0.05 :
                            ...
                            if next_frames_path == 'end':
                                ...
                                #getting height for the right hand sexondary video change x for y depending if serve or forehand
    ...
    for idx, (image1, image2) in enumerate(zip(primary_frames_folder, secondary_frames_folder)):
        ...
        with mp_pose.Pose(
            ...
            min_detection_confidence=0.5) as pose:
                ...
                # Calculate offset between nose landmarks
                ...
                # Calculate error between each landmark by using the nose as a reference point for comparison
                if results1.pose_landmarks and results2.pose_landmarks is not None:
                    for (landmark1, landmark2) in zip(results1.pose_landmarks.landmark, results2.pose_landmarks.landmark):
                           x_error = landmark1.x - landmark2.x - dx
                           y_error = landmark1.y - landmark2.y - dy
                           landmark_error.append([x_error, y_error])
                           err.append([x_error, y_error])
                           mse = mse + norm([x_error, y_error]) ** 2

                else:
                    # Set error value to 0 for no detected landmarks
                    for i in range(0,33):
                        landmark_error.append([0, 0])   
                        #err.append([0, 0])     

                # Set error value to 0 for incorrect detetection 

                if idx == 3 or idx == 5 or idx == 63:
                    landmark_error = []
                    mse = 0
                    for i in range(0,33):
                        landmark_error.append([0, 0])   
    ...
    # Draw different colored landmarks based on landmark errors
            ...
            min_detection_confidence=0.5) as pose:
                ...
                if results1.pose_landmarks and results2.pose_landmarks is not None:
                    for (landmark1, landmark2) in zip(results1.pose_landmarks.landmark, results2.pose_landmarks.landmark):
                           x_error = landmark1.x - landmark2.x - dx
                           y_error = landmark1.y - landmark2.y - dy
                           error = norm([x_error, y_error])
                           landmark_error.append(error)
                else:
                    # Set error value to 0 for no detected landmarks
                    for i in range(0,33):
                        landmark_error.append(0)
                ...
                landmark_error = np.array(landmark_error)    
    ...
    # Extract landmarks
    landmark_image1 = cv2.imread('frames/nadal_serve_frame_0.jpg')
    landmark_image2 = cv2.imread('frames/jonas_serve_frame_0.jpg')
